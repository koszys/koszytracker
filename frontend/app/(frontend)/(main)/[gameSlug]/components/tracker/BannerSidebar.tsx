...
export default function BannerSidebar({ banners, bannerStats, activeBannerId, onBannerChange, gameId }: BannerSidebarProps) {
    return (
        <div className="w-full lg:w-70 xl:w-[320px] lg:shrink-0 flex flex-col gap-4 h-60 max-[1022px]:h-75 lg:h-full overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] scrollbar-none">
                ...
                {banners.map((banner) => {
                    ...
                    return (
                        <button
                            ...
                            className={`cursor-pointer w-full text-left rounded-lg p-4 transition-all duration-200 border relative overflow-hidden flex flex-col min-h-35 max-[1022px]:min-h-60 shrink-0 group ${isActive
                                ? "bg-[#2a2b30] border-[#4a4b52]"
                                : "bg-[#1c1d21] border-[#52525b] hover:border-gray-500"
                                }`}
                        >
                            {banner.cardImage && (
                                <div className={`absolute right-0 top-0 w-1/2 h-full overflow-hidden transition-transform duration-300 group-hover:scale-120 ${isActive ? 'scale-120' : ''}`}>
                            ...
                            <div className="relative z-10 flex flex-col h-full justify-between">
                                ...
                                <p className="text-sm text-white font-medium">{banner.name}</p>
                            </div>
                        </button>
                    );
                })}
